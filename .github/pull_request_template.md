...
## 影響面
- API 行為變化：（有/無，列出來）
- DB schema 變動：（有/無，改了 `db/ddl.sql` / `db/dml.sql` 哪個）
- 前端需要同步調整：（有/無，列出端點）
...
- [ ] DB schema 改 `db/ddl.sql`、種子改 `db/dml.sql`（無 Flyway/migrations），改完用 `docker-compose down -v && up -d` 重建驗證
- [ ] 審計日誌欄位符合 `LOGGING_STANDARDS.md`（`action=` / `result=`）

## 風險與 Rollback
- 風險：
- Rollback：revert PR +（schema 變動是否需手動回滾 `ddl.sql` / `dml.sql`）